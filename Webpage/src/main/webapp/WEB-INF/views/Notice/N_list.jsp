		...
		<table class="n_notice">
			...
			<tbody>
				<c:choose>
					...
					<c:otherwise>
						<c:forEach var="k" items="${noticelist}" varStatus="vs">
							<tr>
								<td>${paging.totalRecord-((paging.nowPage-1)*paging.numPerPage+vs.index)}</td>
								<td><a href="noticeOnelist.do?n_idx=${k.n_idx}&nPage=${paging.nowPage}">${k.n_subject}</span> </td>
								<td>${k.n_writer}</td>
								<td>${k.n_write_date.substring(0,10)}</td>
								<td>${k.n_hit}</td>
							</tr>
						</c:forEach>
					</c:otherwise>
